...
yum update -y \
  ...
  && yum install -y --enablerepo=remi-php74 \
       ...
       nginx yum-cron git vsftpd
systemctl enable nginx && systemctl enable php-fpm && systemctl enable yum-cron && systemctl enable vsftpd
